...
<div class="wrap">
    <div class="crumbs">
        ...
        <span>&rsaquo;</span>
        <b>About Banner Section</b>
    </div>

    <div class="header">
        <div>
            <h1>{{ $about->exists ? 'Edit About Banner' : 'Add About Banner' }}</h1>
            <p>The title and banner image shown at the top of your About page.</p>
        </div>
    </div>
    ...
    <div class="images-row">
        ...
        <div class="image-slot">
            ...
            <div class="video-drop {{ ($about->banner_video ?? null) ? 'has-file filled' : '' }} {{ $errors->has('banner_video') ? 'input-error' : '' }}"
                 id="drop-banner-video" onclick="handleAboutVideoDropClick(this)">
                @if ($about->banner_video ?? null)
                    <video src="{{ asset('storage/' . $about->banner_video) }}" muted playsinline preload="metadata"></video>
                    <button type="button" class="remove-img-btn" onclick="removeAboutUploadedVideo(event)" title="Remove video">
                        <i class="bi bi-x-lg"></i>
                    </button>
            ...
            <input type="file" id="file-banner-video" name="banner_video" accept="video/*" hidden
                   onchange="showAboutBannerVideoFileName(this)">
...
function handleAboutVideoDropClick(el) {
    if (el.classList.contains('filled')) return;
    document.getElementById('file-banner-video').click();
}

function showAboutBannerVideoFileName(input) {
...
function removeAboutUploadedVideo(event) {
    event.stopPropagation();
    const drop = document.getElementById('drop-banner-video');
